...
def apply_k_anonymity_r(dataframe, quasi_identifiers, k_value):
    try:
        ...
        quasi_identifiers_r = StrVector(quasi_identifiers)

        # Create an SdcMicro object with the data
        sdc_obj = sdcmicro.createSdcObj(dat=r_dataframe, keyVars=quasi_identifiers_r)

        # Apply k-anonymity using localSuppression
        anonymized_sdc = sdcmicro.localSuppression(sdc_obj, k=k_value)

        # Extract the manipulated key variables from the SdcMicro object
        anonymized_df = pandas2ri.rpy2py(robjects.r['get.sdcMicroObj'](anonymized_sdc, type="manipKeyVars"))

        # Replace sentinel values with None or other appropriate values
        anonymized_df.replace(-2147483648, None, inplace=True)
...
def apply_l_diversity_r(dataframe, quasi_identifiers, sensitive_column, l_value):
    try:
        ...
        sensitive_column_index = IntVector([dataframe.columns.get_loc(sensitive_column) + 1])

        # Apply l-diversity using the ldiversity function
        ldiversity_result = sdcmicro.ldiversity(sdc_obj, ldiv_index=sensitive_column_index, l_recurs_c=l_value)

        # Convert the anonymized sdcMicro object back to a data frame
        anonymized_df = pandas2ri.rpy2py(robjects.r['get.sdcMicroObj'](ldiversity_result, type="manipKeyVars"))
...
def anonymize():
    ...
    # Determine file extension to handle CSV and TSV files
    file_extension = os.path.splitext(file.filename)[1].lower()

    # Choose the correct delimiter based on the file extension
    ...
    # Parse hierarchy_rules JSON string
    hierarchy_rules = json.loads(request.form.get('hierarchy_rules', '{}'))

    file_path = os.path.join(bp.root_path, 'uploads', file.filename)
    file.save(file_path)

    # Read the uploaded file with the appropriate delimiter
    try:
        dataPd = pd.read_csv(file_path, delimiter=delimiter)
        print(dataPd.head())
    except Exception as e:
        return jsonify({'error': f"Error reading file: {str(e)}"}), 400

    # Check if quasi-identifiers exist in the file
    for qi in quasi_identifiers:
