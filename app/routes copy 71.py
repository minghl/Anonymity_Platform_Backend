...
def apply_differential_privacy(dataframe, epsilon, quasi_identifiers, hierarchy_rules):
    try:
        ...
        for qi in quasi_identifiers:
            if qi in dataframe.columns:
                ...
                if rule == "dates":
                    ...
                    # 添加高斯噪声
                    noise = np.random.normal(0, sigma, size=days_since_epoch.shape)
        ...
        print(f"Differentially Private DataFrame with (epsilon={epsilon}, delta={delta}):")
        print(anonymized_df.head())
