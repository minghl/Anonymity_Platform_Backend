...
def apply_k_anonymity_r(dataframe, quasi_identifiers, k_value, hierarchy_rules):
    try:
        ...
        # 记录匿名化后的 NaN 位置
        anonymized_nan_mask = anonymized_df[quasi_identifiers].isna()

        # 找出匿名化过程中新增的 NaN 值位置
        new_nan_mask = (anonymized_nan_mask & ~original_nan_mask)

        # 删除新增的 NaN 值行
        anonymized_df = anonymized_df[~new_nan_mask.any(axis=1)]

        # Step 2: 删除不满足 k 匿名性的记录
        # 计算等价类的频率
        ...
        print(f"Anonymized DataFrame from R (k={k_value}) after removing classes with size < k:")
        print(anonymized_df.head())
...
def create_k_anonymous_dataframe(dataframe, quasi_identifiers, k, hierarchy_rules):
    """
    Generalize quasi-identifiers in the dataframe to ensure k-anonymity.
    """
    generalized_df = dataframe.copy()
    
    # Apply generalization based on hierarchy rules
    for column in quasi_identifiers:
        hierarchy = hierarchy_rules.get(column, {})
        generalized_df[column] = generalized_df[column].apply(lambda x: generalize_value(x, hierarchy))
    
    # Group by quasi-identifiers to check for equivalence classes
    grouped_df = generalized_df.groupby(quasi_identifiers).filter(lambda x: len(x) >= k)
    
    return grouped_df

def generalize_value(value, hierarchy):
    """
    Generalize a value based on the hierarchy.
    If a more general value exists, return it. Otherwise, return the original value.
    """
    return hierarchy.get(value, value)

def check_l_diversity(equivalence_class, sensitive_column, l_value):
    """
    Check if the equivalence class satisfies l-diversity by ensuring the sensitive_column
    has at least l distinct values.
    """
    unique_sensitive_values = equivalence_class[sensitive_column].nunique()
    return unique_sensitive_values >= l_value

def apply_l_diversity_r(dataframe, quasi_identifiers, sensitive_column, l_value, k_value, hierarchy_rules):
    """
    Apply k-anonymity and l-diversity to the dataframe.
    """
    # Step 1: Ensure k-anonymity by generalizing quasi-identifiers
    k_anonymous_df = create_k_anonymous_dataframe(dataframe, quasi_identifiers, k_value, hierarchy_rules)
    
    # Step 2: Initialize an empty DataFrame to store the final anonymized data
    result_df = pd.DataFrame()

    # Step 3: Iterate over each group (equivalence class) generated by quasi-identifiers
    for _, group in k_anonymous_df.groupby(quasi_identifiers):
        # Check if the current equivalence class meets l-diversity
        if check_l_diversity(group, sensitive_column, l_value):
            # If the equivalence class meets l-diversity, append it to the result
            result_df = pd.concat([result_df, group])
        else:
            # If the equivalence class does not meet l-diversity, apply suppression or further generalization
            print(f"Equivalence class does not meet l-diversity (l={l_value}). Applying suppression.")
            
            # Suppress the sensitive column values for this group
            generalized_group = group.copy()
            generalized_group[sensitive_column] = "Suppressed"
            
            # Optionally, you can also generalize quasi-identifiers further or handle the group differently
            # You can further adjust the quasi-identifiers if needed for the group
            
            # Append the generalized group to the result
            result_df = pd.concat([result_df, generalized_group])
...
def anonymize():
    ...
    try:
        if privacy_model == 'k-anonymity':
            ...
            resultPd = apply_l_diversity_r(dataPd, quasi_identifiers, sensitive_column, l_value, k_value, hierarchy_rules)
            return resultPd.to_json(orient='records')
